...
import { WalletLinkRequestSchema } from "@/contracts/wallet-link";
    ...
    try {
        return await withSession(
            ...
            async (sessionError, session) => {
                ...
                try {
                    const input = await parseJsonBody(request, WalletLinkRequestSchema);
                    const data = await getWalletLinkService().linkWallet(session.getUserId(), input);
